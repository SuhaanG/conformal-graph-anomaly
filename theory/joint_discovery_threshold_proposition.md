...
confirmed directly from `results/published/selection_bias_matrix.csv`
row-by-row, not just the aggregate summary. This is a genuinely striking
pattern: architecturally distinct reconstruction-based detectors, with
opposite-signed degree sensitivity, converge on nearly identical gamma. That
convergence is itself evidence the mechanism there is not primarily about
degree, consistent with (A1) failing on weibo (tau=-0.018, identical across
every detector and seed, confirming the selection filter genuinely does not
shift degree there).

**Correction to "regardless of detector": OCGNN is a sharp exception, and it
is the lead worth following.** Per-seed values (5 seeds):

    seed 0: sdeg=+0.129  gamma=undefined (0 discoveries)
    seed 1: sdeg=-0.105  gamma=0.000   n_disc=5   FDR=0.000
    seed 2: sdeg=-0.203  gamma=0.000   n_disc=22  FDR=0.000
    seed 3: sdeg=-0.217  gamma=0.000   n_disc=12  FDR=0.000
    seed 4: sdeg=-0.205  gamma=2.355   n_disc=39  FDR=0.205

Three of five seeds show exactly zero realized FDR with real, non-trivial
discovery activity (5, 22, and 12 discoveries, all true positives) -- not
the zero-discovery pathology flagged elsewhere in this document, since
discoveries did occur and none were false. One seed shows real inflation
(FDR=0.205), well above nominal but still roughly a third of what every
other detector shows consistently (~0.62-0.65). This is not noise-driven:
the four reconstruction-based detectors have near-zero cross-seed variance
in gamma_at_bh (std 0.00-0.17); ocgnn's variance is an order of magnitude
larger (three exact zeros, one 2.36), which is itself informative --
ocgnn's relationship to whatever weibo's selection shift operates through
is qualitatively different, not just quantitatively noisier.

OCGNN is architecturally distinct from the other four detectors in the
matrix: it is a one-class, hypersphere-distance detector (Deep SVDD-style),
not a reconstruction-error detector. The other four (dominant_ours,
dominant_pygod, gae, anomalydae) all score by some form of reconstruction
error and all show the flat ~7 pattern regardless of sign. This architectural
split -- one clear outlier, one shared architecture family, both showing
internally consistent behavior -- is the most promising lead currently
available for what weibo's exchangeability violation actually runs through.
It is not degree (ruled out by (A1) failing uniformly and by the sdeg-sign
irrelevance among the other four). Whatever it is, ocgnn's scoring mechanism
is far less sensitive to it. Investigating what specifically differs between
reconstruction-error scoring and hypersphere-distance scoring on this graph
is the natural next step, and it is cheap: no new experiment is required,
only inspection of ocgnn's score construction against whatever covariate
turns out to matter.

Weibo is also the one dataset with a measurable contamination signal
(exposure->score r=0.111 on degree-normalized scores per
