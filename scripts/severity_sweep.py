...
def run_severity_trial(p_an, alpha, seed, n_epochs, device, n_calib=270):
    ...
    n_calib is now a FIXED constant (270, matching the validated Step 5 setup)
    rather than derived from the clean-calibration pool, because that pool
    collapses to near-zero at higher p_an values -- and we don't need it here,
    since this sweep only runs the adversarial condition (top-exposed nodes),
    not the clean condition."""
    cfg = GraphGenConfig(
